            ...
            if llm_provider == "gemini":
                ...
                try:
                    ...
                    if "429" in err_msg or "quota" in err_msg.lower():
                        logger.error(f"Gemini Quota Exceeded: {err_msg}")
                        # Return a specific error string that outer loops can identify
                        return f"Error: Gemini API quota exceeded (429). {err_msg}"
                    print("Gemini Error:", e)
                    raise ValueError(f"[{llm_provider}] returned invalid response content: {err_msg}")
...
def get_prompts():
    ...
    """
    prompts_file = os.path.join(utils.root_dir(), "prompts.txt")
    if not os.path.exists(prompts_file):
        logger.warning(f"prompts.txt not found at {prompts_file}, using default prompts")
        return ""
...
def generate_script(
    ...
    # Use advanced strategist prompt from prompts.txt
    prompts_content = get_prompts()
    
    # Simple estimate of words needed for the duration (approx 2.5 - 3 words per second)
    word_count_min = int(video_duration * 2.5)
    word_count_max = int(video_duration * 3.0)
    duration_instruction = f"\n- CRITICAL: The total script MUST contain enough spoken words to fill approximately {video_duration} seconds when spoken at a natural pace (Target Word Count: {word_count_min}-{word_count_max} words)."

    if prompts_content:
        # Replace placeholders
        prompt = prompts_content.replace("{video_subject}", video_subject)
        prompt = prompt.replace("{paragraph_number}", str(paragraph_number))
        if language:
            prompt = prompt.replace("{language}", language)
        else:
            prompt = prompt.replace("{language}", "the same as the video subject")
        
        # Append duration instruction
        prompt += duration_instruction
    else:
...
""".strip()
    ...
    def format_response(response):
        ...
        # If the response contains our new strict headers, extract only the script
        if "FULL VIDEO SCRIPT" in response.upper():
            # Look for the section between "FULL VIDEO SCRIPT" and the next header
            pattern = r"FULL VIDEO SCRIPT:?\s*\n*(.*?)(?:\n+\s*\d\.?|\n+\s*PEXELS|\n+\s*KEYWORDS|\n+\s*DESCRIPTION|\n+\s*YOUTUBE|\Z)"
            match = re.search(pattern, response, re.IGNORECASE | re.DOTALL)
            if match:
                response = match.group(1).strip()
        
        # Strip trailing metadata if headers were missing or match failed
        # Added numeric prefixes (3., 4.) to the patterns to catch merged text
        strip_patterns = [
            r"(.*?)(?:\n+\s*(?:\d\.?\s*)?PEXELS\s+VIDEO\s+SEARCH\s+KEYWORDS.*)",
            r"(.*?)(?:\n+\s*(?:\d\.?\s*)?YOUTUBE\s+DESCRIPTION\s+&?\s+HASHTAGS.*)",
            r"(.*?)(?:\n+\s*(?:\d\.?\s*)?KEYWORDS:.*)",
            r"(.*?)(?:\n+\s*(?:\d\.?\s*)?DESCRIPTION:.*)",
            r"(.*?)(?:(?:\d\.?\s*)?PEXELS\s+VIDEO\s+SEARCH\s+KEYWORDS.*)", # No newline version
            r"(.*?)(?:(?:\d\.?\s*)?YOUTUBE\s+DESCRIPTION\s+&?\s+HASHTAGS.*)", # No newline version
        ]
        ...
        # Split into paragraphs and join
        ...
        try:
            ...
            if response and "Error:" in response:
                if "429" in response or "quota" in response.lower():
                    ...
                    logger.warning(f"Quota exceeded detected, waiting {wait_time}s before retry {i + 1}...")
                    time.sleep(wait_time)
                    continue
                else:
                    logging.error(f"LLM error: {response}")
