...
export function ParticleAnimation() {
  ...
  useEffect(() => {
    ...
    const initSketch = () => {
      if (!containerRef.current || !(window as any).p5 || !(window as any).gsap) return
      ...
      sketchRef.current = new (window as any).p5(sketch)
    }
  ...
  return (
    <>
      ...
      <div
        ref={containerRef}
        className="particle-container w-full h-full flex items-center justify-center"
      />
    </>
  )
}
